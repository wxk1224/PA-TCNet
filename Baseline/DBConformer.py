...
class DBConformer(nn.Module):
    def __init__(self, emb_size=40, 
                 tem_depth=5, chn_depth=5, 
                 chn=22, n_classes=2, n_samples=1000):
        super().__init__()
        ...
        self.embedding = PatchEmbeddingTemporal(
            ...
            kernel_size=63,
            radix=1,
            patch_size=self.patch_size,  # needs to be divisible by the number of time points
            time_points=n_samples,  # number of time points
            num_classes=n_classes  # number of classes
        )
        # Default 125, needs to be divisible by the number of time points

        self.channel_embedding = PatchEmbeddingSpatial(spa_dim=16, emb_size=40)  # Default 16
        self.P = n_samples // self.patch_size  
        self.C = chn  
        self.D = emb_size
    ...
    def forward(self, x):  # x: (B, 1, C, T)
        ...
        if self.posemb_flag:
            ...
            x_embed_spatial = x_embed_spatial + self.pos_embedding_spatial  # spatial positional 
