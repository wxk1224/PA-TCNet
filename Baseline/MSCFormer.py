...
class PatchEmbeddingCNN(nn.Module):
    def __init__(self, f1=16, pooling_size=52, dropout_rate=0.5, number_channel=22):
        super().__init__()
        if pooling_size < 1: pooling_size = 1
...
class PositionalEncoding(nn.Module):
    def __init__(self, embedding, length=2000, dropout=0.1):
        super().__init__()
...
class MSCFormer(nn.Module):
    def __init__(self, 
        ...
        self.emb_size = f1 * 3  # Original logic: 3 branches concat
        ...
        self.position = PositionalEncoding(self.emb_size, length=2000, dropout=0.1)
        ...
        self.classifier = ClassificationHead(self.emb_size, self.number_class)

    def forward(self, x):
        # x.shape = [batch, channels, samples] or [batch, 1, channels, samples]
        if x.dim() == 3:
            x = x.unsqueeze(1)
        x = self.cnn(x)
        b, l, e = x.shape
        ...
        # Add class token
        cls_token = torch.zeros((b, 1, e), requires_grad=True).to(x.device)
